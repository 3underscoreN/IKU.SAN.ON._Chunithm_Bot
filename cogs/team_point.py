...
def _load_state():
    ...
    try:
        ...
        logger.warning(
            "Invalid persisted team point channel id in config table. Ignoring value."
        )
    ...
    try:
        ...
        logger.warning(
            "Invalid persisted team point message id in config table. Ignoring value."
        )
...
class TeamPointCog(commands.GroupCog, name="teampoint"):
    ...
    async def update_team_points(self):
        ...
        embed = info_embed(
            ...
            fields=[
                (player, f"{score} 分", False) for player, score in team_scores.items()
            ],
        )
    ...
    async def update_now(self, interaction: discord.Interaction):
        ...
        await interaction.response.send_message(
            "已觸發團隊積分更新。請稍後片刻後查看。", ephemeral=True
        )
        await self.update_team_points()

    @app_commands.command(
        name="set_channel", description="設定或更新團隊積分訊息的位置。"
    )
    ...
    async def confirm_overwrite_tp_msg(self, interaction: discord.Interaction) -> bool:
        ...
        try:
            await self.bot.wait_for(
                "interaction",
                check=lambda i: (
                    i.data.get("custom_id") == "confirm_overwrite_tp_msg"
