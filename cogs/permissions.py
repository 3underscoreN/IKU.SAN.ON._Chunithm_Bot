...
from utils.permissions import Permission, PermissionMapping
...
class PermissionsCog(commands.GroupCog, name="permissions"):
    ...
    @app_commands.describe(
        role="要設定權限的 Discord 身分組", permission_level="欲設定的權限等級"
    )
    ...
    async def add_permission(
        ...
        if permission_level not in (Permission.MEMBER, Permission.ADMIN):
            embed = error_embed(description=f"無效的權限等級：{permission_level}。")
            await interaction.response.send_message(embed=embed, ephemeral=True)
            return
        ...
        await interaction.response.send_message(embed=embed, ephemeral=True)
        logger.info(
            f"User {interaction.user} added {perm_name} permission to role {role.id}"
        )
    ...
    async def remove_permission(
        ...
        if existing_perm is None:
            embed = warning_embed(description=f"{role.mention} 目前沒有設定任何權限。")
            await interaction.response.send_message(embed=embed, ephemeral=True)
            return
        ...
        embed = info_embed(
            title="✅ 權限已移除", description=f"已移除 {role.mention} 的所有權限設定。"
        )
        await interaction.response.send_message(embed=embed, ephemeral=True)
    ...
    async def view_permissions(self, interaction: discord.Interaction):
        ...
        if not role_permissions:
            embed = info_embed(
                title="身分組權限", description="目前沒有任何身分組權限設定。"
            )
            await interaction.response.send_message(embed=embed, ephemeral=True)
            return
