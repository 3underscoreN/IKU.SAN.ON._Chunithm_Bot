...
def colornorm(
    min_color: tuple[int], max_color: tuple[int], multiplier: float
) -> tuple[int]:
    """
    ...
    return tuple(
        int(min_c + (max_c - min_c) * multiplier)
        for min_c, max_c in zip(min_color, max_color)
    )
...
def generate_self_calendar(year: int, month: int, marked_days: set[int]) -> io.BytesIO:
    ...
    draw.text(
        (width - padding - len(generated_at) * 10, height - padding + 4),
        generated_at,
        fill=text_color,
        font=font_count,
    )
...
def generate_all_calendar(
    year: int, month: int, count_of_days: MutableMapping[int, int]
) -> io.BytesIO:
    """
    Generate a calendar image for the specified year and month, marking the specified days.
    ...
    count_of_days_norm = {
        day: powernorm(count, min_count, max_count, 0.3)
        for day, count in count_of_days.items()
    }
    ...
    for row_idx, week in enumerate(month_days):
        for col_idx, day in enumerate(week):
            ...
            if count > 0:
                ...
                this_text_color = (
                    text_color_on_max if color_intensity > 0.7 else text_color
                )
                draw.rectangle([x1, y1, x2, y2], fill=color)

            draw.rectangle(
                [x1, y1, x2, y2], outline=border_color, width=border_width
            )  # border

            draw.text(
                (x1 + 10 + border_width, y1 + 10 + border_width),
                str(day),
                fill=this_text_color,
                font=font,
            )  # calendar day number

            if (
                count > 0 and count in count_values_sorted[:2]
            ):  # top 2 counts get the win image
                img.paste(
                    win_image, (x1 + cell - 48, y1 + cell - 48), win_image
                )  # win image for max count

            draw.text(
                (x1 + cell - 48 + 16, y1 + cell - 48 - 16),
                f"{str(count)}",
                fill=this_text_color,
                font=font_count,
            )  # count text
